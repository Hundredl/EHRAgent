...
from pyehr.datasets.loader.datamodule import EhrDataModule
from pyehr.pipelines import DlPipeline
from pyehr.datasets.loader.load_los_info import get_los_info
...
class Pipeline:
    ...
    def __init__(self,
        ...
        self.config = {
            ...
            'output_dim': 1 if task in ['outcome', 'los'] else 2,
            'epochs': epochs,
            'patience': patience,
            'task': task,
            'seed': seed,
    ...
    def train(
        ...
        ckpt_url = os.path.join(ckpt_path)
    ...
    def predict(
        ...
        performance = {k: v if type(v) != torch.Tensor else v.item() for k, v in pipeline.test_performance.items()}
    ...
    def execute(self, model_path: Optional[str] = None) -> dict:
        """
