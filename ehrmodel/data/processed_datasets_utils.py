...
def forward_fill_pipeline(
    ...
    for name, group in grouped:
        ...
        patient_x = []
        patient_y = []
        patient_missing_mask = pd.isna(sorted_group[features].values).tolist()
        ...
        sorted_group = sorted_group.ffill()
        # print(sorted_group)
...
def normalize_dataframe(
    ...
    # Apply filter_outlier function
    # train_df.loc[:, normalize_features] = train_df.loc[:, normalize_features].apply(filter_outlier)
    # val_df.loc[:, normalize_features] = val_df.loc[:, normalize_features].apply(filter_outlier)
    # test_df.loc[:, normalize_features] = test_df.loc[:, normalize_features].apply(filter_outlier)
...
def save_record_time(
    ...
    for _, group in grouped:
        sorted_group = group.sort_values(by=['RecordTime'], ascending=True)
        record_time = sorted_group['RecordTime'].astype(str).tolist()
